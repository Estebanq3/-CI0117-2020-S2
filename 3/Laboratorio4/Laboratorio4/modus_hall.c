...
void *prudes() {
    ...
    //Llegada de la cola, es decir los hilos que estaban esperando entrar a la zona crìtica, van llegando
    ...
    if(!_prudesCounter) //Si no hay más prudes, prudes = 0
        if(_heathensCounter) //Y hay heathens
            sem_post(&_heathensTurn); //Libera la ejecución de los heathens, es el turno de los heathens, banderazo verde a heathens para ocupar la zona crìtica
    ...
    } else {sem_post(&_prudesTurn);} //De lo contrario, sigue corriendo los prudes, turno de los Prudes

    printf("\nPRUDE sale de la zona crìtica\nFaltan %d Prudes | %d Heathens\n", _prudesCounter, _heathensCounter); //prudes encerrados en la zona crìtica
...
int main() {
    ...
    if(nHeathens > nPrudes)
        ...
        start = 0; //SemPrudes comienza como 1 (¡consígalo! N)
    
    _tidHeathens = (pthread_t *) malloc(nHeathens * sizeof(pthread_t)); //Vector de hilo que se inicializará para paganos
    _tidPrudes = (pthread_t *) malloc(nPrudes * sizeof(pthread_t)); //Vector de hilos a inicializar para los mojigas.
